...
class LDCTNet_NoFreq(nn.Module):
    """Variant A: 两条路径都使用原始输入（不做高低频分离）"""
    def __init__(self):
        super().__init__()
        # 不使用 LearnableGaussianBlur
        # 高频分支
    ...
    def forward(self, x):
        # 不做分频：高、低频分支都用原始输入 x
        fea_high = self.conv_hr4(self.conv_hr3(self.conv_hr2(self.conv_hr1(x))))
        fea_low = self.conv_lr4(self.conv_lr3(self.conv_lr2(self.conv_lr1(x))))
...
if __name__ == "__main__":
    ...
    out = m(inp)
    print("NoFreq out shape:", out.shape)  # expect [1,1,256,256]
