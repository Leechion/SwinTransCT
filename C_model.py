...
class LDCTNet_NoResidualFusion(nn.Module):
    """Variant C: 将 residual 融合替换为普通卷积融合（无残差连接）"""
    def __init__(self):
        super().__init__()
    ...
    def forward(self, x):
        ...
        # 普通卷积融合（无残差）
        fused = self.fuse_conv(h_feat + l_feat)
...
if __name__ == "__main__":
    m = LDCTNet_NoResidualFusion()
    inp = torch.randn(1,1,256,256)
    out = m(inp)
    print("NoResidualFusion out shape:", out.shape)
