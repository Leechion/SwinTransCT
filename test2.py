...
def test_model(args):
    ...
    csv_save_path = os.path.join(args.result_dir, "test_metrics_B.csv")  # 指标CSV
    group_img_save_path = os.path.join(args.img_save_dir, "best_worst_samples_group.png")  # 组图

    # 3. 加载测试集（复用训练时的Dataset，确保预处理一致）
    print("\n[1/5] 加载测试集...")
    test_pairs = get_pair_list(args.data_dir, split="val")  # 读取测试集配对列表
    ...
    # 4. 初始化模型与指标计算器
    print("\n[2/5] 初始化模型与工具...")
    model = LDCTNet_NoFreq().to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_path", type=str, default="./checkpoints/best_model_A.pth",
                        help="训练好的模型路径")
