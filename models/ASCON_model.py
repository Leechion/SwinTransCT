...
class ASCONModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if opt.netG == 'ESAU':
            self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout,
                                          ...
                                          self.gpu_ids, opt, initialize_weights=False)
        else:
            self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.normG, not opt.no_dropout,
                                          opt.init_type, opt.init_gain, opt.no_antialias, opt.no_antialias_up,
                                          self.gpu_ids, opt, initialize_weights=True)
        ...
        if self.isTrain:
            ...
            self.netProjection_online = networks.define_F(opt.input_nc, opt.netF, opt.normG, not opt.no_dropout,
                                                          opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids,
                                                          opt, initialize_weights=initialize_weights).to(self.device)
            self.netProjection_target = networks.define_F(opt.input_nc, opt.netF, opt.normG, not opt.no_dropout,
                                                          opt.init_type, opt.init_gain, opt.no_antialias, self.gpu_ids,
                                                          opt, initialize_weights=initialize_weights).to(self.device)
        ...
        if self.isTrain:
            ...
            self.optimizer_R = torch.optim.SGD(
                list(self.netOnline.parameters()) + list(self.netProjection_online.parameters()) + list(
                    self.netPredictor.parameters()), lr=self.opt.lr)
    ...
    def MAC_Net(self, real_B, fake_B):
        ...
        feat_q_pool_1, _, _, _ = self.netProjection_online(patch_size, feat_q_1, self.num_patches, sample_ids,
